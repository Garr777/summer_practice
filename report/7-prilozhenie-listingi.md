...
Потоки данных между компонентами приведены на рисунке Б.1. Схема служит картой
к последующим разделам приложения: каждый листинг соответствует одному из узлов.

![Потоки данных между компонентами комплекса](images/dataflow.png)

*Рисунок Б.1 — Потоки данных: от телеметрии кластера до показателей сравнения
конфигураций*

Схема отвечает на вопрос «что откуда берётся», тогда как
[рисунок 4.1](4-proektnaya-chast/4-metodologiya-soprovozhdeniya.md) описывает
порядок действий во времени. Три обстоятельства, существенные для оценки
методической корректности, считываются именно с неё.

**Три политики назначения запросов взаимозаменяемы.** Действующая конфигурация
(`configured`), эвристика (`heuristic`) и обучаемая модель (`model`) сходятся в
единый интерфейс и подаются в имитационное ядро одинаково; ядро не располагает
сведениями о том, какая из них применена. Отсюда следует, что базовая линия и
предлагаемый метод сравниваются **одним кодом на одних данных**, а различие
результатов относится к политике, а не к способу измерения. Это же свойство
означает, что подключение новой политики не затрагивает ни ядро, ни интерфейс.

**Защиты от утечки целевой переменной локализованы в двух узлах.**
Дедупликация прогонов выполняется в `dataset.py` (раздел Б.2), вычисление
признаков со сдвигом — в `train.py` (раздел Б.4). Оба узла выделены на схеме:
ошибка в любом из них завысила бы оценку качества модели, не проявившись ни в
одном показателе.

**Резерв и потребление учитываются раздельно.** Имитационное ядро резервирует
ёмкость по запросу, а фактическое потребление отслеживает параллельно
(раздел Б.8). Без этого разделения переподписание ёмкости было бы
принципиально ненаблюдаемым.

| Узел схемы | Раздел приложения |
|---|---|
| `dataset.py` — дедупликация и временные признаки | Б.2 |
| `policy.py` — эвристика | Б.3 |
| `train.py` — признаки со сдвигом | Б.4 |
| `train.py` — квантильная модель | Б.5 |
| `pools.py` — описание пула | Б.6 |
| `scenario.py` — модель числа ядер | Б.7 |
| `engine.py` — планирование и резервирование | Б.8 |
| Показатели сравнения | Б.9 |

## Б.2. Устранение смещения выборки
...
## Б.3. Действующая эвристика как базовая линия
...
## Б.4. Признаки, известные до запуска задачи
...
## Б.5. Квантильная модель и сравнение с базовой линией
...
## Б.6. Описание пула исполнителей
...
## Б.7. Модель числа ядер и растяжения длительности
...
## Б.9. Показатели оценки конфигурации
...
## Б.10. Интерфейс исследования сценариев
...
| Сетка узлов | заливка полосы — зарезервировано по запросу, метка — фактическое потребление | наблюдаемость переподписания (раздел Б.8) |
...
свыше 330 тысяч рабочих нагрузок [11], то есть рассматриваемый контур не
является нетипичным.
...
недооценённых по памяти прогонов (раздел Б.9)
измеряет реальный, а не гипотетический риск.

## Б.11. Оценка объёма реализации
